...
function feedDirective (feedService, $compile, $templateCache, $http) {
    ...
    function feedDirectiveController ($scope, $element, $attrs, $timeout, $interval) {
        ...
        function renderTemplate (templateHTML, feedsObj) {
            ...
            $scope.currentDate = $scope.feeds[0].updated;
        }
...
function feedService ($window, $q, $sce, feedCache) {
    ...
    function sanitizeFeedEntry (feedEntry) {
        ...
        var properties = [
            ...
            {indexName: 'image', possibleIndexNames: ['image']},
            {indexName: 'updated', possibleIndexNames: ['updated']}
        ];
            ...
            property.possibleIndexNames.forEach(function (contentIndex) {
              //console.log(contentIndex)
                ...
                if (feedEntry[contentIndex]) {
                    ...
                    var content = typeof feedEntry[contentIndex] === 'string' ? feedEntry[contentIndex] : feedEntry[contentIndex][0];
                    if (!content) {
...
//    }
//}
angular.module('feeds').run(['$templateCache', function($templateCache, $scope) {
  'use strict';
  
  $templateCache.put('feed-list.html',
    "<div class='demo-menu-example' aria-label='podcast api' role='listbox'>" +
    "    <h4>Top Tech itunes podcasts</h4> " +
    "    <h5>{{currentDate | date:'fullDate'}} </h5>" +
    "    <div class='demo-menu-item' ng-repeat='feed in feeds | orderBy:publishedDate:reverse'>" +
    "                <a target='_new' ng-href='{{feed.link}}'><img ng-src='{{feed.image.content}}' class='podcast-img'/>{{feed.title}}</a>" +
    "    </div>" +
    "</div>"
  );
